  ...
  return (
    <div className="bg-white rounded-lg p-5 border" style={{ borderColor: '#E8DDD4' }}>
      <div className="flex items-baseline justify-between mb-1 flex-wrap gap-1">
        ...
        {pctChange !== null && (
          <span className="text-sm font-bold" style={{ color: pctChange >= 0 ? '#10B981' : '#EF4444' }}>
        ...
        <table className="w-full text-xs min-w-[420px]">
          <thead>
            <tr className="border-b" style={{ borderColor: '#F3F4F6' }}>
              <th className="text-left py-1.5 font-medium" style={{ color: '#9CA3AF' }}>Month</th>
              <th className="text-right py-1.5 font-medium" style={{ color: '#9CA3AF' }}>This Year</th>
              <th className="text-right py-1.5 font-medium" style={{ color: '#9CA3AF' }}>Last Year</th>
              <th className="text-right py-1.5 font-medium" style={{ color: '#9CA3AF' }}>Δ%</th>
            </tr>
          </thead>
          <tbody>
            {points.map(p => {
              ...
              return (
                <tr key={p.label} className="border-b last:border-0" style={{ borderColor: '#F9FAFB' }}>
                  <td className="py-1">{p.label}</td>
                  <td className="py-1 text-right">{fmt(p.current, metric)}</td>
                  <td className="py-1 text-right" style={{ color: '#9CA3AF' }}>{fmt(p.prior, metric)}</td>
                  <td className="py-1 text-right" style={{ color: delta === null ? '#9CA3AF' : delta >= 0 ? '#10B981' : '#EF4444' }}>
