...
export default function CodClient({ orders }: { orders: CodOrder[] }) {
  ...
  return (
    <div className="space-y-3">
      {error && (
        <p className="text-sm mb-1" style={{ color: C.criticalStrong }}>{error}</p>
      )}
      {list.length === 0 && (
        <p className="text-sm" style={{ color: 'var(--admin-subtle)' }}>No COD orders yet.</p>
      )}
      {list.map(order => {
        const status = order.cod_status || 'pending'
        return (
          <div key={order.id} className="bg-[var(--admin-surface)] rounded-lg border p-4" style={{ borderColor: 'var(--admin-border)' }}>
