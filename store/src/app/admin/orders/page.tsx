      ...
      <div className="space-y-3">
        {orders.map(order => (
          <div key={order.id} className="bg-white rounded-lg border overflow-hidden" style={{ borderColor: '#E8DDD4' }}>
            ...
            >
              <div>
                <p className="font-medium text-sm">#{order.id.slice(0, 8).toUpperCase()} — {order.customer_name}</p>
              ...
              <div className="border-t p-4 bg-gray-50" style={{ borderColor: '#E8DDD4' }}>
                ...
                {(order.items as OrderItem[]).map((item, i) => (
                  <p key={i} className="text-sm text-gray-600 mb-1">
                    {item.product_name} × {item.quantity} ({item.size}, {item.color}) — PKR {item.price.toLocaleString()}
                  </p>
                ))}
                <div className="mt-4">
